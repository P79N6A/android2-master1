...
public class DownLoadManager {
     ...
     */
    public int addTask(int fatherId, String fatherTitle, String fatherImg, int childId, String chileUrl, String chileTitle) {
        return addTask(fatherId, fatherTitle, fatherImg, childId, chileUrl, chileTitle, null);
}
     ...
     * @return -1 : 文件已存在 ，0 ： 已存在任务列表 ， 1 ： 添加进任务列表
     */
    public int addTask(int fatherId, String fatherTitle, String fatherImg, int childId, String chileUrl, String chileTitle, String childFilePath) {
        //TODO WHWH CHANGE
        // if (TaskID == null) {
        //     TaskID = fileName;
        //}
        ...
        //fatherId, fatherTitle, fatherImg, childId, chileUrl, chileTitle
        SQLDownLoadInfo downloadinfo = new SQLDownLoadInfo();
        downloadinfo.setUserID(userID);
        downloadinfo.setFatherId(fatherId);
        downloadinfo.setFatherTitle(fatherTitle);
        downloadinfo.setFatherImg(fatherImg);
        downloadinfo.setChildId(childId);
        downloadinfo.setChildTitle(chileTitle);
        downloadinfo.setChildUrl(chileUrl);
        downloadinfo.setChildDownSize(0);
        downloadinfo.setChildSize(0);
        downloadinfo.setIsFinish(0);

        if (childFilePath == null) {
            downloadinfo.setChildFilePath(FileHelper.getFileDefaultPath() + "/" + "." + childId + ".mp4");
        } else {
            downloadinfo.setChildFilePath(childFilePath);
        }
    ...
    private int getAttachmentState(String TaskID, String fileName, String filepath) {
        ...
        File file = null;
        if (filepath == null) {
            file = new File(FileHelper.getFileDefaultPath() + "/" + TaskID);
            if (file.exists()) {
                return -1;
            }
        } else {
     ...
     * @return
     */
    public ArrayList<DownVideoBean> getAllTask() {
        ArrayList<DownVideoBean> taskInfolist = new ArrayList<DownVideoBean>();
        int listSize = taskList.size();
        for (int i = 0; i < listSize; i++) {
            DownLoader deletedownloader = taskList.get(i);

            SQLDownLoadInfo sqldownloadinfo = deletedownloader.getSQLDownLoadInfo();
            if (sqldownloadinfo.getIsFinish() == 0) {
                DownVideoBean taskinfo = new DownVideoBean();
                taskinfo.setFatherId(sqldownloadinfo.getFatherId());
                taskinfo.setFatherTitle(sqldownloadinfo.getFatherTitle());
                taskinfo.setFatherImg(sqldownloadinfo.getFatherImg());
                taskinfo.setChildId(sqldownloadinfo.getChildId());
                taskinfo.setChildUrl(sqldownloadinfo.getChildUrl());
                taskinfo.setChildTitle(sqldownloadinfo.getChildTitle());
     ...
     * 根据id获取下载任务列表中某个任务
     */
    public DownVideoBean getTaskInfo(String taskID) {
        DownLoader downloader = getDownloader(taskID);
        if (downloader == null) {
            return null;
        }
