...
public class PopNewManyPay extends PopupWindow implements View.OnClickListener {
    ...
    private OnManyPayCliclListener mOnManyPayCliclListener;
    //自己是否能够观看 0 能  1 不能
    private int MYSELF_ALLOW = 0;
    private double mPrice = 0.00;

    public void setOnManyPayCliclListener(OnManyPayCliclListener onManyPayCliclListener) {
        mOnManyPayCliclListener = onManyPayCliclListener;
    }
    ...
    public PopNewManyPay(final Context context, String title, double price, int pay) {
        super(context);
        mContext = context;
        mPrice = price;
        MYSELF_ALLOW = pay;
        mLayoutInflater = LayoutInflater.from(MyApplication.mApplication);
        View view = mLayoutInflater.inflate(R.layout.pop_new_many_pay, null, false);
        initView(view);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.pay_cancel:
                ...
                break;
            case R.id.many_pay:
                if (mOnManyPayCliclListener != null) {
                    mOnManyPayCliclListener.onManyPayClick();
                }
                break;
            case R.id.pay_ok:
    ...
    public void setCouponInfo(String coupon,String couponID) {
        if (!"".equals(couponID)) {
            double coupon_price = 0;
            try {
                coupon_price = Integer.parseInt(coupon);
            }catch (Exception e)
            {
                coupon_price = 0;
            }
            mPayCoursePrice.setText(sub(mPrice,coupon_price) + "元");
            mCouponInfo.setText(setCouponText("-" + coupon + "元","#FFAD1E"));
        }else{
    ...
    public interface OnManyPayCliclListener {
        void onManyPayClick();
    }
