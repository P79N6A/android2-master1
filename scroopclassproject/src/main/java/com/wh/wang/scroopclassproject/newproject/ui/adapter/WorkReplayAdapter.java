...
public class WorkReplayAdapter extends RecyclerView.Adapter<WorkReplayAdapter.VideoReplayViewHolder> {
    private String mUserId;
    private List<WorkDetailsEntity.CommentBean.ChildBean> mReplyBeanList;
    private Context mContext;
    private LayoutInflater mInflater;

    public WorkReplayAdapter(List<WorkDetailsEntity.CommentBean.ChildBean> replyBeanList, Context context) {
    ...
    public void onBindViewHolder(VideoReplayViewHolder holder, final int position) {
        if(holder!=null&&mReplyBeanList.get(position)!=null){
            final WorkDetailsEntity.CommentBean.ChildBean childBean = mReplyBeanList.get(position);
            String remark_name = subName(childBean.getNickname())+" ";
            String content = remark_name+"回复 "+subName(childBean.getRe_name())+": "+childBean.getContent();
            ...
            holder.mTime.setText(childBean.getShijian());
            if(childBean.getUser_id().equals(mUserId)){
                ...
                holder.mDelete.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if(mOnSecondDeleteClickListener!=null){
                            mOnSecondDeleteClickListener.onSecondDelClick(position,childBean.getId());
                        }
                    }
                });
            }else{
            ...
            holder.mReplay.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(mOnSecondReplayClickListener!=null){
                        mOnSecondReplayClickListener.onReplayClick(childBean.getId(),childBean.getNickname());
                    }
                }
            });
        }
